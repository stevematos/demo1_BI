...
class ETL:
    ...
    def transform(self,values_players,information_players):
        values_players = values_players | "poniendo id al los valores" >> beam.ParDo(MapBigQueryRow('id'))
        information_players = information_players | "poniendo id a la informacion" >>  beam.ParDo(MapBigQueryRow('id'))

        records = ({'values':values_players,'information':information_players} | 'group_by_id' >> beam.CoGroupByKey())
